...
#####    是否包含字符串三种新方法
&emsp;&emsp;传统上，JavaScript只有 indexOf 方法，可以用来确定一个字符串是否包含在另一个字符串中。ES6又提供了三种新方法。
+ &emsp;includes()：返回布尔值，表示是否找到了参数字符串。
+ &emsp;startsWith()：返回布尔值，表示参数字符串是否在源字符串的头部。
+ &emsp;endsWith()：返回布尔值，表示参数字符串是否在源字符串的尾部。  
